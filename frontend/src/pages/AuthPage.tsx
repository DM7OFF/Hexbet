...
import { Gamepad2, TrendingUp, Zap, Shield, Eye, EyeOff, Trophy, Dice5, Target, CircleDot } from 'lucide-react';
...
export default function AuthPage({ onAuth }: AuthPageProps) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setWins(prev => {
        const newWin = generateWin(counterRef.current++);
        return [newWin, ...prev.slice(0, 14)];
      });
    }, 1800);
  ...
  return (
    <div className="min-h-screen bg-[#07090F] text-white overflow-hidden relative">
      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        <div className="hidden lg:flex flex-col flex-1 p-10 gap-8">
            ...
            <div className="grid grid-cols-5 gap-3">
              {GAMES.map((game) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${game.color} flex items-center justify-center group-hover:shadow-lg transition-all duration-300`}>
                    ...
                    {/* Hover glow */}
                    <div className={`absolute inset-0 rounded-xl bg-gradient-to-br ${game.color} opacity-0 group-hover:opacity-5 transition-opacity duration-300`} />
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex-1 min-h-0">
            ...
            <div ref={feedRef} className="space-y-2 overflow-hidden max-h-52">
              {wins.map((win, i) => (
                <div
                  key={win.id}
          ...
          <div className="relative w-full max-w-sm">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm mb-7">
                {mode === 'login'
                  ? 'Connectez-vous pour accéder à vos jeux et gains.'
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {mode === 'register' && (
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-600 focus:outline-none focus:border-purple-500 focus:bg-white/8 transition-all duration-200"
                    />
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="••••••••"
                      required
                      className="w-full px-4 py-3 pr-12 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-600 focus:outline-none focus:border-purple-500 focus:bg-white/8 transition-all duration-200"
                    />
                    <button
                      type="button"
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 text-white font-bold text-sm hover:opacity-90 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 shadow-lg hover:shadow-pink-500/30 disabled:opacity-60 disabled:cursor-not-allowed mt-2"
                >
                  {loading ? (
              ...
              <button className="mt-4 w-full py-3 rounded-xl bg-white/5 border border-white/10 text-sm text-gray-300 font-medium hover:bg-white/10 transition-all duration-200 flex items-center justify-center gap-3">
      ...
      {/* Game click prompt */}
      {showGamePrompt && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full bg-gradient-to-r from-pink-500/90 to-purple-600/90 backdrop-blur-md text-white text-sm font-bold shadow-2xl animate-bounce border border-white/20">
